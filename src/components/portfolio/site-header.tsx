...
export function SiteHeader() {
	...
	const sectionHref = (href: string) => (isHome ? href : `/${locale}${href}`);
							...
							{ui.nav.map((item) => (
								<li key={item.href} className="hidden sm:list-item">
									<a
										...
										className={`editorial-link inline-flex min-h-11 items-center px-1.5 text-sm text-text-muted no-underline hover:text-text-primary sm:px-2 ${focusRing}`}
									>
										{item.label}
									</a>
								</li>
